...
def setup_cli():
    ...
    # Add subcommand for removing devices
    remove_parser = subparsers.add_parser("remove", help="Remove a device")
    remove_parser.add_argument("--name", required=True, help="Device name to remove")
    
    return parser


# OPTIONAL FUNCTIONS - Additional functionality

def remove_device(inventory, device_name):
    """
    Removes a device from the inventory list based on device name.
